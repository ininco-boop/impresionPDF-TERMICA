...
function imprimirRawBT(texto, cufe) {
  // 1) Imprimir el texto con la fuente nativa de la impresora (nítido, como la primera versión)
  const textEncoded = encodeURI(texto);
  const S = "#Intent;scheme=rawbt;";
  const P = "package=ru.a402d.rawbtprinter;end;";
  window.location.href = "intent:" + textEncoded + S + P;

  // 2) Luego de un momento, imprimir el QR como imagen aparte
  setTimeout(() => {
    const qrContenido = "https://dgi-fep.mef.gob.pa/Consultas/FacturasPorCUFE/" + cufe;
    const dataUrl = construirImagenQR(qrContenido);
